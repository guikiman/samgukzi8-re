...
export function processMonthlyFreeOfficerVisits(store: GameStore, random: () => number = Math.random): FreeOfficerVisit[] {
    const visits: FreeOfficerVisit[] = [];
    const free = store.getAllOfficers().filter(o =>
        o.status === OfficerStatus.FREE && o.runtime.isAlive && o.cityId !== null);

    for (const officer of free) {
        const city = store.getCity(officer.cityId!);
        if (!city || !city.ownerId) continue;
        ...
        const acceptChance = Math.min(MAX_ACCEPT_CHANCE,
            BASE_ACCEPT_CHANCE + (leaderCharisma - 60) * 0.005 + Math.min(0.1, leaderFame / 3000));
...
export function applyJoin(store: GameStore, officerId: string, factionId: string, cityId: string, leaderFame: number): void {
    ...
    if (faction && !faction.officers.includes(officer.id as never)) {
        store.updateFaction(factionId, { officers: [...faction.officers, officer.id as never] });
    }
    const city = store.getCity(cityId);
    if (city && !city.officerIds.includes(officer.id as never)) {
        store.updateCity(cityId, { officerIds: [...city.officerIds, officer.id as never] });
    }
}

/** 플레이어 선택 — 맞이하기 */
export function acceptVisit(store: GameStore, visit: FreeOfficerVisit): FreeOfficerVisit {
...
/** 플레이어 선택 — 사절하기 (재야 유지) */
export function declineVisit(visit: FreeOfficerVisit): FreeOfficerVisit {
    visit.joined = false;
