import type { Personality, OfficerID, FactionID, CityID } from "./types";
...
export class RebellionSystem {
  ...
  private calculateDiscontent(ctx: RebellionContext): number {
    let discontent = 0;
    discontent += (100 - ctx.officerLoyalty) * 0.3;
    discontent += ctx.officerAmbition * 0.25;
    discontent += (100 - ctx.factionStability) * 0.2;
    if (ctx.isGovernor) discontent += 10;
    if (ctx.officerRank <= 3) discontent += 15;

    const personalityMods: Record<string, number> = {
  ...
  suppressRebellion(
    rulerMight: number,
    rulerIntelligence: number,
    rebelMight: number,
    garrisonLoyalty: number,
  ): { suppressed: boolean; description: string } {
