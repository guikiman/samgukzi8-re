...
import type { DuelCard, DuelCardType } from './duel_deck_builder';
...
export interface DuelState {
    ...
    readonly playerFocusBoost: boolean;   // FOCUS 버프 활성 여부
...
export interface DuelTurnResult {
    ...
    readonly playerDamageDealt: number;   // 플레이어가 AI에게 준 데미지
    readonly aiDamageDealt: number;       // AI가 플레이어에게 준 데미지
 ...
 */
const COUNTER_TABLE: Record<DuelCardType, DuelCardType | null> = {
...
const CARD_SPIRIT_COST: Record<DuelCardType, number> = {
    ATTACK: 0,
    DEFENSE: 0,
    CRITICAL: 2,
    DODGE: 0,
...
export class DuelTurnProcessor {
    ...
    processTurn(
        ...
        const actualPlayerCard = playerSpirit < 0
            ? { type: 'ATTACK' as DuelCardType, damage: CARD_DAMAGE.ATTACK, spiritCost: 0 }
            : playerCard;
        const actualAiCard = aiSpirit < 0
            ? { type: 'ATTACK' as DuelCardType, damage: CARD_DAMAGE.ATTACK, spiritCost: 0 }
            : aiCard;

        if (playerSpirit < 0) playerSpirit = state.playerSpirit; // Spirit 부족시 원복
        if (aiSpirit < 0) aiSpirit = state.aiSpirit;
        ...
        if (newPlayerHp <= 0) winner = 'ai';
        else if (newAiHp <= 0) winner = 'player';
    ...
    selectAICard(state: DuelState, aiMight: number): DuelCard {
        const available: DuelCardType[] = ['ATTACK', 'DEFENSE', 'DODGE', 'FOCUS'];
