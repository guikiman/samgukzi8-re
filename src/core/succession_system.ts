...
export class SuccessionSystem {
  determineSuccessor(ctx: SuccessionContext): SuccessionResult {
    const bloodlineCandidates = ctx.candidates.filter(
      (c) =>
        c.relationToLeader === "son" ||
  ...
  calculateSuccessorLoyalty(
    successor: SuccessionCandidate,
    otherCandidates: SuccessionCandidate[],
  ): Map<OfficerID, number> {
    const loyaltyMap = new Map<OfficerID, number>();
    for (const candidate of otherCandidates) {
      if (candidate.id === successor.id) continue;
      let loyalty = candidate.loyalty;
      if (candidate.ambition > 70) loyalty -= 15;
      if (candidate.relationToLeader === "son" && successor.relationToLeader !== "son") loyalty -= 10;
