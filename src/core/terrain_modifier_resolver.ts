...
export interface TerrainModifier {
    readonly moveCost: number;         // 기본 이동력 소모(1)
    readonly attackPenalty: number;    // 공격력 패널티(%)
    readonly defenseBonus: number;     // 방어력 보너스(%)
    readonly avoidBonus: number;       // 회피 보너스(%)
}

const TERRAIN_BASE: Record<TerrainType, TerrainModifier> = {
...
const UNIT_CLASS_MODIFIERS: Record<UnitClass, Partial<Record<TerrainType, TerrainModifier>>> = {
    INFANTRY: {
