...
/** 군주(세력) 평판 등급 산출 */
export function getLeaderReputationVisual(
    faction: { fame: number; infamy: number } | null | undefined,
): ReputationVisual {
    if (!faction) {
...
export function getOfficerReputationVisual(
    officer: { fame: number; infamy: number } | null | undefined,
): ReputationVisual {
    if (!officer) {
        return { label: '무명', icon: '·', color: '#888', title: '평판을 알 수 없음' };
    }
    const score = officer.fame - officer.infamy * 2;
    if (score >= 400) return { label: '天下', icon: '✨', color: '#4caf50', title: `천하에 이름이 알려짐 (명성 ${officer.fame})` };
    if (score >= 250) return { label: '名士', icon: '🌟', color: '#e8c35a', title: `일대 명사 (명성 ${officer.fame})` };
    if (officer.infamy >= 100) return { label: '凶名', icon: '🔥', color: '#e05a5a', title: `흉명이 자자함 (악명 ${officer.infamy})` };
    return { label: '무명', icon: '·', color: '#9a9a9a', title: `특별한 평판 없음 (명성 ${officer.fame})` };
}

/** 상단 바 칩 한 개 HTML 생성 */
export function renderReputationChip(vis: ReputationVisual): string {
