...
const SHIP_STATS: Record<ShipType, ShipDef> = {
...
const SHIP_COUNTER: Record<ShipType, ShipType> = {
    LOUSCHUAN: 'MENGCHONG',
    MENGCHONG: 'WARSHIP',
    WARSHIP: 'PATROL',
    PATROL: 'TRANSPORT',
...
const TERRAIN_PENALTY: Record<NavalTerrain, Record<ShipType, number>> = {
...
const WEATHER_MOVEMENT_PENALTY: Record<BattleWeather, number> = {
    SUNNY: 1.0, CLOUDY: 0.9, RAIN: 0.7, SNOW: 0.5, STORM: 0.3, FOG: 0.6,
};

const WEATHER_VISION_PENALTY: Record<BattleWeather, number> = {
...
export class NavalCombatSystem {
    ...
    getShipCounter(attackerType: ShipType, defenderType: ShipType): number {
        if (SHIP_COUNTER[attackerType] === defenderType) return 1.5;
        if (SHIP_COUNTER[defenderType] === attackerType) return 0.7;
        return 1.0;
    }

    calculateNavalDamage(
        attackerShip: ShipType,
        defenderShip: ShipType,
        attackerStats: OfficerStats,
        _defenderStats: OfficerStats,
    ...
    getVisionRange(shipType: ShipType, weather: BattleWeather): number {
        const baseVision: Record<ShipType, number> = {
