...
export interface SupplyLineState {
    ...
    readonly moralePenalty: number;       // 단절 시 턴당 사기 감소량
}
...
export class SupplyLineCutter {
    ...
    checkSupplyLine(
        ...
        const queue: { q: number; r: number; dist: number }[] = [{ q: baseQ, r: baseR, dist: 0 }];
        const unitKey = `${unitQ},${unitR}`;

        let found = false;
        let blockedPosition: HexCoord | null = null;
        ...
        while (queue.length > 0) {
            ...
            for (const [dq, dr] of HEX_DIRECTIONS) {
                ...
                if (enemySet.has(nKey) || enemyControlTiles.has(nKey)) {
                    if (!blockedPosition) blockedPosition = { q: nq, r: nr };
                    continue;
                }
