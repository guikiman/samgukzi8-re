...
const STATUS_ICONS: Record<StatusType, number> = {
  burning: 0,
  confused: 1,
  poisoned: 2,
  taunted: 3,
...
export class StatusIconPacker {
   ...
   * 상태 타입 → UV 좌표 변환
   */
  getUV(type: StatusType, sheetWidth: number, sheetHeight: number): { u: number; v: number; w: number; h: number } {
   ...
   * 모든 아이콘 UV 맵 반환
   */
  getAllUVs(sheetWidth: number, sheetHeight: number): Record<StatusType, { u: number; v: number; w: number; h: number }> {
    const result = {} as Record<StatusType, { u: number; v: number; w: number; h: number }>;
