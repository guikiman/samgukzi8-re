...
function lastCaptiveEvent(store: GameStore, officerId: string): string | null {
    ...
    const markers = edge.history.filter(h =>
        h.event === CAPTURE_MARKER_EVENT || h.event === ESCAPE_EVENT || h.event === RELEASE_EVENT,
    );
...
export function imprisonCaptive(store: GameStore, officerId: string, holdingCityId: string): void {
...
export function judgeEscape(intelligence: number, roll: number): boolean {
