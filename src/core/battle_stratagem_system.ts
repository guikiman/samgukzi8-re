...
import type { HexCoord, OfficerStats } from './types';
...
export const STRATAGEMS: Record<StratagemType, Stratagem> = {
...
const WEATHER_EFFECT: Record<Weather, Record<string, number>> = {
...
export class BattleStratagemManager {
    private activeEffects: Map<string, StratagemEffect> = new Map();
    ...
    getAvailableStratagems(officerId: string, intelligence: number, spirit: number): Stratagem[] {
        const unlocked: StratagemType[] = [];
        if (intelligence >= 30) unlocked.push('ROCKFALL', 'AMBUSH', 'FLANK');
        if (intelligence >= 50) unlocked.push('TRAP', 'FIRE_ATTACK');
        if (intelligence >= 70) unlocked.push('FALSE_RETREAT');
        if (intelligence >= 85) unlocked.push('BURN_GRANARY');
    ...
    processActiveEffects(unitPositions: Map<string, HexCoord>): StratagemEffect[] {
        ...
        const activeDamage: StratagemEffect[] = [];
    ...
    checkEncircled(unitCoord: HexCoord, enemyTiles: HexCoord[], radius: number = 1): boolean {
        const neighbors = this.getNeighborCoords(unitCoord);
        const enemySet = new Set(enemyTiles.map(t => `${t.q},${t.r}`));
        let surrounded = 0;

        for (const n of neighbors) {
            if (enemySet.has(`${n.q},${n.r}`)) surrounded++;
        }

        return surrounded >= radius * 4; // 6타일 중 4개 이상이 적이면 포위
    }

    private getNeighborCoords(coord: HexCoord): HexCoord[] {
        const dirs = [
