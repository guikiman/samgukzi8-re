...
export class PostFXManager {
    private presets: Map<string, ShaderPreset> = new Map();
    ...
    private register(id: string, source: string, uniforms: string[]): void {
        this.presets.set(id, { id, source, uniforms, enabled: true, intensity: 1.0 });
    }

    getShader(id: string): ShaderPreset | undefined { return this.presets.get(id); }
    enable(id: string, val: boolean): void {
        const p = this.presets.get(id);
        if (p) this.presets.set(id, { ...p, enabled: val });
    }
    setIntensity(id: string, val: number): void {
        const p = this.presets.get(id);
        if (p) this.presets.set(id, { ...p, intensity: val });
    }
}
