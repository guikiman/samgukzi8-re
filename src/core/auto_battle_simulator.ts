...
const TERRAIN_DEFENSE: Record<AutoBattleTerrain, number> = {
    PLAIN: 0,
    FOREST: 0.1,
    MOUNTAIN: 0.2,
    RIVER: 0.05,
...
export interface AutoBattleRoundLog {
    ...
    criticals: Array<{ side: 'attacker' | 'defender'; who: string }>;
    note?: string;
}
...
export function computeCombatPower(unit: AutoBattleUnit): number {
...
function computeRoundDamage(
    attacker: AutoBattleUnit,
    defender: AutoBattleUnit,
    attackerTile: BattlefieldTile | undefined,
    defenderTile: BattlefieldTile | undefined,
    siege: boolean,
    ...
    if (attackerTile) {
        ...
        if (highGround) damage = Math.floor(damage * ELEVATION_BONUS);
    }
 ...
 */
export function simulateAutoBattle(sides: AutoBattleSides): AutoBattleResult {
    const attacker = { ...sides.attacker };
    const defender = { ...sides.defender };
    const roundLogs: AutoBattleRoundLog[] = [];
    let round = 0;
    ...
    while (round < MAX_ROUNDS &&
        ...
        const criticals: Array<{ side: 'attacker' | 'defender'; who: string }> = [];
        if (atk.critical) criticals.push({ side: 'attacker', who: attacker.commanderName });
        if (def.critical) criticals.push({ side: 'defender', who: defender.commanderName });
    ...
    const winner: 'attacker' | 'defender' =
        attacker.soldiers > defender.soldiers && attacker.soldiers > 0 ? 'attacker' : 'defender';

    const totalCasualties =
        (sides.attacker.soldiers - Math.max(0, attacker.soldiers)) +
 ...
 * 전투력 비율 + 난수로 단판 승부.
 */
export function resolveInstantBattle(sides: AutoBattleSides): AutoBattleResult {
    ...
    return {
        ...
        totalCasualties: Math.floor(
            sides.attacker.soldiers * attackerLossRate + sides.defender.soldiers * defenderLossRate,
        ),
        roundLogs: [],
