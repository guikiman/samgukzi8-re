...
export interface FormationLayout {
  ...
  readonly offsets: readonly { readonly dq: number; readonly dr: number; readonly label: string }[];
}

const FORMATIONS: Record<FormationType, FormationLayout> = {
  line: {
    type: "line",
    offsets: [
      { dq: 0, dr: 0, label: "C" },
...
export class HexFormationRenderer {
   ...
   * 진형의 중심 기준 모든 좌표 반환
   */
  getFormationTiles(type: FormationType, centerQ: number, centerR: number): { q: number; r: number; label: string }[] {
