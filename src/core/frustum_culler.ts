...
export class FrustumCuller {
  ...
  update(projectionMatrix: Float32Array, viewMatrix: Float32Array): void {
    ...
    for (const p of this.planes) {
      ...
      if (len > 0) {
        (p as { nx: number }).nx = p.nx / len;
        (p as { ny: number }).ny = p.ny / len;
        (p as { nz: number }).nz = p.nz / len;
        (p as { d: number }).d = p.d / len;
      }
    }
  }
  ...
  private multiplyMat4(out: Float32Array, a: Float32Array, b: Float32Array): void {
    for (let i = 0; i < 4; i++) {
