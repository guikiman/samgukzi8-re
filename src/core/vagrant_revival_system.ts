 ...
 */
export function convertToFactionVagrant(
    store: GameStore,
    factionId: FactionID,
): VagrantConversion | null {
    ...
    const sorted = [...members].sort((a, b) => {
        if (a.id === faction.leaderId) return -1;
        if (b.id === faction.leaderId) return 1;
 ...
 * 방랑군 세력 ID 목록 조회
 */
export function getVagrantFactionIds(store: GameStore): FactionID[] {
 ...
 * 도시 점령 처리부에서 호출
 */
export function clearVagrantOnCityGain(store: GameStore, factionId: FactionID): void {
    const faction = store.getFaction(factionId);
