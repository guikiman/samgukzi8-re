...
/** 이미 의형제인지 */
function alreadySworn(store: GameStore, aId: string, bId: string): boolean {
 ...
 */
export function findPactCandidates(store: GameStore, minAffinity: number = PACT_MIN_AFFINITY): Array<{ aId: string; bId: string; affinity: number }> {
    const officers = store.getAllOfficers().filter(o =>
        o.runtime.isAlive
        && (o.status !== OfficerStatus.FREE || o.factionId === null)
        && (o.status === OfficerStatus.FREE ? o.factionId === null : true)).slice(0, 300);

    const candidates: Array<{ aId: string; bId: string; affinity: number }> = [];
    const seen = new Set<string>();

    for (const a of officers) {
        for (const b of officers) {
            if (a.id >= b.id) continue; // 중복 쌍 제거 (정렬 키)
            const key = `${a.id}|${b.id}`;
            if (seen.has(key)) continue;
            seen.add(key);
            if (alreadySworn(store, a.id, b.id)) continue;

            // 같은 세력 또는 한쪽 재야
            const sameFaction = a.factionId !== null && a.factionId === b.factionId;
            const oneFree = a.factionId === null || b.factionId === null;
            if (!sameFaction && !oneFree) continue;
            if (a.factionId !== null && b.factionId !== null && a.factionId !== b.factionId) continue;
 ...
 */
export function executeSwornBrotherPact(
    store: GameStore,
    aId: string,
    bId: string,
): SwornBrotherPactRecord | null {
    const a = store.getOfficer(aId);
    const b = store.getOfficer(bId);
    if (!a || !b) return null;
    if (alreadySworn(store, aId, bId)) return null;
 ...
 */
export function processMonthlySwornBrotherPacts(store: GameStore): SwornBrotherPactReport {
    const messages: string[] = [];
    const candidates = findPactCandidates(store);
    if (candidates.length === 0) return { pacts: [], messages };
    ...
    if (!record) return { pacts: [], messages };
