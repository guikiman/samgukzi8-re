...
export class WandererArmyManager {
    private state: WandererArmyState | null = null;
    ...
    addOfficer(officerId: OfficerID): boolean {
        if (!this.state?.isActive) return false;
        if (this.state.officerIds.includes(officerId)) return false;
    ...
    uprise(
        ...
        if (success) {
            const newFactionId = `faction_wanderer_${this.state.leaderId}` as FactionID;
            this.state = {
                ...this.state,
