...
export class CompressedTextureLoader {
  private supportedFormats = new Set<CompressedFormat>();
  ...
  detectSupportedFormats(gl: WebGL2RenderingContext): CompressedFormat[] {
    ...
    if (exts.includes("WEBGL_compressed_texture_s3tc")) this.supportedFormats.add("s3tc");
    if (exts.includes("WEBGL_compressed_texture_etc")) this.supportedFormats.add("etc");
    if (exts.includes("WEBGL_compressed_texture_astc")) this.supportedFormats.add("astc");
    if (exts.includes("WEBGL_compressed_texture_pvrtc")) this.supportedFormats.add("pvrtc");
   ...
   */
  async loadFromBlob(gl: WebGL2RenderingContext, url: string): Promise<WebGLTexture | null> {
    const img = new Image();
    img.src = url;
    await img.decode();
