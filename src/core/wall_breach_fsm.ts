...
export class WallBreachFSMTransition {
    ...
    processBreach(
        ...
        adjacentUnitIds: string[],
    ): BreachEvent {
        // 상태 전이 결정
        let newPhase: BattlePhase = currentPhase;
        switch (breachType) {
            case 'GATE_DESTROYED':
                // 성문 파괴: FIELD → SIEGE
                if (currentPhase === 'FIELD') newPhase = 'SIEGE';
                break;
            case 'WALL_BREACHED':
                // 성벽 붕괴: SIEGE → INNER
                if (currentPhase === 'SIEGE') newPhase = 'INNER';
                break;
    ...
    detectBreach(gateDurability: number, wallDurability: number, isGateTarget: boolean): BreachType | null {
        if (isGateTarget && gateDurability <= 0) return 'GATE_DESTROYED';
        if (!isGateTarget && wallDurability <= 0) return 'WALL_BREACHED';
        return null;
    }
}
