...
export class PregnancySimulator {
  private pregnancies = new Map<string, PregnancyEvent>();

  calculatePregnancyChance(motherAge: number, marriageDuration: number, existingChildren: number): number {
    if (motherAge < 16 || motherAge > 45) return 0;
    if (existingChildren >= 6) return 0.01;
  ...
  checkBirth(pregnancy: PregnancyEvent, currentTurn: number): { birth: boolean; stillbirth: boolean; twins: boolean } {
    if (currentTurn < pregnancy.dueTurn) return { birth: false, stillbirth: false, twins: false };
  ...
  generateBirthEvent(motherName: string, fatherName: string, twins: boolean, stillbirth: boolean): string {
    if (stillbirth) return `${motherName}이(가) 사산했다... 모두가 슬퍼한다.`;
    if (twins) return `${motherName}이(가) 쌍둥이를 출산했다! ${fatherName}의 가문에 경사가 났다!`;
