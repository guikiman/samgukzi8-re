...
export class EventFeedbackEffects {
    private shake: ShakeState = { intensity: 0, decay: 6, time: 0, freq: 28 };
    ...
    trigger(kind: FeedbackKind): void {
        const intensityByKind: Record<FeedbackKind, number> = {
            VENGEANCE_SUCCESS: 9,
            VENGEANCE_FAIL: 5,
            RESCUE: 6,
    ...
    update(dt: number): { x: number; y: number } {
        const s = this.shake;
        if (s.intensity <= 0.1 || !this.shakeEnabled) return { x: 0, y: 0 };
        s.time += dt;
    ...
    disableShake(): void { this.shakeEnabled = false; }
    enableShake(): void { this.shakeEnabled = true; }
    disableSound(): void { this.soundEnabled = false; }
    enableSound(): void { this.soundEnabled = true; }
}
