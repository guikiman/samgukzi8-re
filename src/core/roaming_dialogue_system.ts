...
/** 방문자 유형별 대화 씬 생성 */
export function getRoamingDialogue(visitorType: string, cityName: string, factionName: string | null): RoamingDialogueData {
...
export function roamingVisitorIcon(visitorType: string): string {
    switch (visitorType) {
        case 'SAGE': return '🧙';
        case 'HERMIT': return '🍃';
 ...
 */
export function resolveRoamingDialogue(
    store: GameStore,
    visitorType: string,
    cityId: string,
    optionId: string,
    roll: number = Math.random(),
): RoamingDialogueResolution {
    const city = store.getCity(cityId);
    if (!city) return { message: '방문 기록을 찾을 수 없습니다', effects: [] };
