...
import type { OfficerID, FactionID, CityID, IGameStore } from './types.js';
...
export interface SupplyRoute {
    ...
    readonly guardLevel: number;  // 0-100 경비 수준
}
...
export class RoamingContentSystem {
    ...
    monitorSupplyRoutes(playerOfficerId: string): SupplyRoute[] {
        ...
        if (!player || player.factionId) return []; // 세력 소속이면 방랑군 아님
        ...
        for (const city of allCities) {
            ...
            for (const neighbor of neighbors) {
                if (neighbor.ownerId !== city.ownerId) continue;
    ...
    private getNeighborCities(cityId: string): any[] {
        const city = this.store.getCity(cityId);
        if (!city) return [];
        return (city as any).neighbors?.map((nid: string) => this.store.getCity(nid)).filter(Boolean) ?? [];
    }
}
