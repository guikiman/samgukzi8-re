...
export interface DebateState {
    ...
    readonly playerLogicShield: number;    // Logic Shield 잔여량 (지력차 기반)
    readonly aiLogicShield: number;
    readonly turn: number;
    readonly maxTurns: number;
    readonly playerCards: DebateCard[];
    readonly aiCards: DebateCard[];
    readonly currentTheme: DebateTheme;     // 이번 설전의 전장 테마
...
export interface DebateTurnResult {
    ...
    readonly playerDamage: number;  // AI가 플레이어에게 준 데미지
    readonly aiDamage: number;      // 플레이어가 AI에게 준 데미지
    readonly themeMatch: boolean;
    readonly shieldAbsorbedPlayer: number; // 플레이어 Logic Shield가 흡수한 양
 ...
 */
const COUNTER_MAP: Record<DebateCardType, DebateCardType> = {
    REASON: 'SOPHISTRY',
    SOPHISTRY: 'THREAT',
    THREAT: 'REASON',
};
...
/** 카드 타입별 레이블 */
const CARD_LABELS: Record<DebateCardType, string> = {
    REASON: '도리',
    THREAT: '위압',
    SOPHISTRY: '궤변',
};
...
/** 카드 타입과 테마 매칭 매핑 */
const THEME_MATCH: Record<DebateTheme, DebateCardType> = {
    REASONING: 'REASON',
    ELOQUENCE: 'THREAT',
...
/** 테마 이름 */
const THEME_LABELS: Record<DebateTheme, string> = {
    REASONING: '도리론',
    ELOQUENCE: '웅변론',
    SOPHISTRY_THEME: '궤변론',
};
...
/** 기본 카드 위력 */
const BASE_CARD_POWER: Record<DebateCardType, number> = {
    REASON: 12,
    THREAT: 12,
    SOPHISTRY: 12,
};
...
export class DebateCardEngine {
    ...
    startDebate(
        ...
        // 테마 랜덤 선택
        const themes: DebateTheme[] = ['REASONING', 'ELOQUENCE', 'SOPHISTRY_THEME'];
     ...
     * 지력 기반 카드 덱 생성
     */
    private generateCards(intelligence: number, count: number): DebateCard[] {
        const cards: DebateCard[] = [];
        const types: DebateCardType[] = ['REASON', 'THREAT', 'SOPHISTRY'];
    ...
    executeTurn(state: DebateState, playerCardIndex: number): DebateState {
        ...
        if (shieldAbsorbedPlayer > 0) descParts.push(`Logic Shield ${shieldAbsorbedPlayer} 흡수!`);
        if (shieldAbsorbedAi > 0) descParts.push(`AI Shield ${shieldAbsorbedAi} 흡수!`);
        ...
        if (newPlayerLogicHp <= 0) winner = 'ai';
        else if (newAiLogicHp <= 0) winner = 'player';
    ...
    selectAICard(state: DebateState): number {
        if (state.aiCards.length === 0) return 0;
