...
function findEdgeBetween(store: GameStore, a: string, b: string): RelationshipEdge | undefined {
 ...
 */
export function applyRecruitFailure(store: GameStore, recruiterId: string, targetId: string): void {
    if (recruiterId === targetId) return;
    if (!store.getOfficer(recruiterId) || !store.getOfficer(targetId)) return;
 ...
 */
export function getRecruitAffinityModifier(store: GameStore, recruiterId: string, targetId: string): number {
