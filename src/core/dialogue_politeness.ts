...
export interface DialogueTemplate {
    readonly formal: string;    // $Formal 노드
    readonly neutral: string;  // $Neutral
    readonly informal: string;  // $Informal
    readonly rude: string;     // $Rude
}
...
export class DialoguePolitenessMask {
    ...
    getPolitenessLevel(speakerId: string, targetId: string): PolitenessLevel {
        ...
        if (!speaker || !target) return 'NEUTRAL';

        const rankDist = Math.abs((speaker.rank as number) - (target.rank as number));
        const affinity = this.getAffinity(speakerId, targetId);
        const personality = (speaker as any).personality ?? 'normal';

        if (rankDist >= 3 && affinity < 50) return 'FORMAL';
        if (affinity >= 70) return 'INFORMAL';
        if (affinity < 20 || personality === 'brutal') return 'RUDE';
        if (rankDist <= 2 && affinity >= 40) return 'INFORMAL';
        return 'NEUTRAL';
    }
    ...
    private getAffinity(a: OfficerID, b: OfficerID): number {
