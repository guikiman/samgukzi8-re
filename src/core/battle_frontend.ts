...
import { HexMapCanvasRenderer, HexTile } from './hex_map_canvas_renderer.js';
...
export interface BattleState {
    ...
    actionLog: string[];
    highlightedPath: { q: number; r: number }[];
}
...
export class BattleFrontend {
    private renderer: HexMapCanvasRenderer;
    ...
    moveUnit(unitId: string, q: number, r: number): boolean {
        ...
        if (cost > unit.movementPoints) return false;
    ...
    attackUnit(attackerId: string, targetQ: number, targetR: number): void {
        ...
        const damage = Math.floor(
            (attacker.baseAttack * attacker.soldiers) /
        ...
        for (const enemy of enemies) {
            ...
            if (nearest && minDist <= 2) {
                ...
                const dmg = Math.floor(
                    (enemy.baseAttack * enemy.soldiers) /
    ...
    private calcMoveRange(unit: BattleUnit): { q: number; r: number }[] {
        const range: { q: number; r: number }[] = [];
        ...
        for (let dq = -maxDist; dq <= maxDist; dq++) {
            for (let dr = -maxDist; dr <= maxDist; dr++) {
                ...
                const occupied = this.state.units.some(u =>
                    u.unitId !== unit.unitId && u.position.q === q && u.position.r === r
                );
                if (!occupied) range.push({ q, r });
            }
        }
        return range;
    }

    private calcAttackRange(unit: BattleUnit): { q: number; r: number }[] {
        const range: { q: number; r: number }[] = [];
        ...
        for (const [dq, dr] of dirs) {
            const q = unit.position.q + dq;
            const r = unit.position.r + dr;
            const enemy = this.state.units.some(u =>
                u.unitId !== unit.unitId && u.position.q === q && u.position.r === r
            );
            if (enemy) range.push({ q, r });
        }
        return range;
    }
    ...
    private renderPhaseOverlay(ctx: CanvasRenderingContext2D): void {
        // Phase indicator at top center
        const phaseNames: Record<BattlePhase, string> = {
            DEPLOYMENT: '⚔️ 배치 페이즈',
            PLAYER_TURN: '🎯 아군 턴',
            ENEMY_TURN: '👹 적군 턴',
            RESULT: '🏆 전투 종료',
