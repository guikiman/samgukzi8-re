...
export interface HellConstraintReport {
    ...
    taxLeaked: number;
    /** 탈영한 무장 */
    deserted: Array<{ officerId: string; officerName: string; cityId: string }>;
    /** 반란한 도시 */
    revolted: Array<{ cityId: string; cityName: string; fromFactionName: string }>;
}

/** 스토어가 지옥 난이도인지 확인 */
export function isHellDifficulty(store: GameStore): boolean {
 ...
 */
export function processHellConstraints(
    store: GameStore,
    random: () => number = Math.random,
): HellConstraintReport {
    const report: HellConstraintReport = { taxLeaked: 0, deserted: [], revolted: [] };
    if (!isHellDifficulty(store)) return report;
    ...
    for (const officer of store.getAllOfficers()) {
        if (!officer.factionId || !officer.runtime.isAlive) continue;
        const faction = store.getFaction(officer.factionId);
        if (!faction || faction.leaderId === officer.id) continue;
        if (officer.loyalty > HELL_DESERTION_LOYALTY) continue;
    ...
    for (const city of store.getAllCities()) {
        if (!city.ownerId) continue;
        if (city.developmentStats.publicOrder > HELL_REVOLT_ORDER) continue;
