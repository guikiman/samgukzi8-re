...
export interface PsychologyState {
    ...
    hasTrauma: boolean;
    loyaltyDelta: number; // 이번 달 충성도 변동량
    willResign: boolean;
    willDefect: boolean;
}

export class OfficerPsychologyEngine {
    private burnout: Map<OfficerID, number> = new Map();
    private paranoia: Map<OfficerID, number> = new Map();
    private trauma: Map<OfficerID, boolean> = new Map();
    private resignationHistory: OfficerID[] = [];
    private defectHistory: OfficerID[] = [];

    private static readonly BURNOUT_THRESHOLD = 80;
    private static readonly PARANOIA_THRESHOLD = 70;
     ...
     * 편집증 발동
     */
    triggerParanoia(officerId: OfficerID, amount = 20): void {
    ...
    processMonthlyUpdate(
        officerId: OfficerID,
        baseLoyalty: number,
        workload: number, // 0~100, 이번 달 업무량
    ): PsychologyState {
        ...
        // 충성도 변동
        let loyaltyDelta = 0;
        if (burnoutLevel > 50) loyaltyDelta -= Math.floor(burnoutLevel * 0.1);
        if (paranoiaLevel > 40) loyaltyDelta -= Math.floor(paranoiaLevel * 0.05);
        ...
        if (burnoutLevel >= OfficerPsychologyEngine.BURNOUT_THRESHOLD) {
            ...
            if (willResign) this.resignationHistory.push(officerId);
        }
        ...
        if (paranoiaLevel >= OfficerPsychologyEngine.PARANOIA_THRESHOLD) {
            ...
            if (willDefect) this.defectHistory.push(officerId);
        }
