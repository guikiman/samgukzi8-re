...
export class TacticalAILogic {
    private static readonly ZOC_RANGE = 1; // ZOC 적용 헥사 거리
    ...
    evaluatePosition(
        unit: TacticalUnit,
        allyPositions: HexCoord[],
        enemyPositions: HexCoord[],
        terrainTypes: Map<string, string>, // "q,r" → terrain type
    ): TacticalEvaluation {
        ...
        if (terrain === 'mountain') threatLevel -= 0.1;
        if (terrain === 'river') threatLevel += 0.2;
    ...
    calculateZOCBypass(
        ...
        while (queue.length > 0 && nodeCount < 1000) {
            ...
            for (const neighbor of this.hexNeighbors(current)) {
                ...
                if (visited.has(nKey) || zocSet.has(nKey)) continue;
                visited.add(nKey);
     ...
     */
    coordinateJointAttack(
        primaryUnit: TacticalUnit,
        allyUnits: TacticalUnit[],
        targetPosition: HexCoord,
    ): { canExecute: boolean; participants: string[]; expectedDamage: number } {
        const participants: string[] = [primaryUnit.id];
        const targetNeighbors = this.hexNeighborsCoord(targetPosition);

        for (const ally of allyUnits) {
            if (ally.id === primaryUnit.id) continue;
     ...
     */
    evaluateCavalryCharge(
        cavalry: TacticalUnit,
        target: HexCoord,
        enemyPositions: HexCoord[],
    ): { canCharge: boolean; retreatPath: HexCoord[]; remainingMP: number } {
    ...
    private hexNeighborsCoord(center: HexCoord): HexCoord[] {
        const dirs = [
    ...
    private countNearby(pos: HexCoord, targets: HexCoord[], range: number): number {
        return targets.filter(t => this.hexDistance(pos, t) <= range).length;
    }

    private reconstructPath(parent: Map<string, HexCoord>, start: HexCoord, end: HexCoord): HexCoord[] {
    ...
    private straightLinePath(start: HexCoord, goal: HexCoord): HexCoord[] {
        return [start, goal];
    }
