...
type PaletteMap = ReadonlyMap<string, PaletteEntry>;

/** 시나리오 세력 고유색 → 색약 친화 대체 (Okabe-Ito 계열 명도차 보정) */
const DEUTERANOPIA: PaletteMap = new Map<string, PaletteEntry>([
...
const TRITANOPIA: PaletteMap = new Map<string, PaletteEntry>([
...
export const PATTERN_OPTIONS = ['none', 'hatch', 'dots', 'border'] as const;
...
export const PATTERN_LABELS: Record<PatternOption, string> = {
    none: '없음',
    hatch: '사선 무늬',
    dots: '점 무늬',
    border: '굵은 테두리',
...
export function convertFactionColor(hex: string, mode: ColorblindMode): string {
...
export function factionSymbol(hex: string, mode: ColorblindMode): string | null {
...
export function makeFactionColorResolver(mode: ColorblindMode): (hex: string) => string {
    return (hex: string) => convertFactionColor(hex, mode);
}
...
): { color: string; symbol: string | null; patternCss: string | null } {
    const color = convertFactionColor(hex, mode);
    const symbol = factionSymbol(hex, mode);
    let patternCss: string | null = null;
    if (pattern === 'hatch') {
