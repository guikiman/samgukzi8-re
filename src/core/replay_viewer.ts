...
export class ReplayViewer {
    private logs: ReplayActionLog[] = [];
    private units = new Map<string, ReplayUnitState>();
    private effects: ReplayEffect[] = [];
    private cursor = 0;
    private stepTimer = 0;
    ...
    /** 재생 속도 — 액션 간 간격 (ms) */
    private stepIntervalMs = 450;
    private readonly flashDurationMs = 500;

    private readonly cb: ReplayViewerCallbacks;
    ...
    /** 재생 시작 */
    play(): void {
        if (this.logs.length === 0 || this.finished) return;
        this.playing = true;
    ...
    get actionCount(): number { return this.logs.length; }
    ...
    update(dt: number): void {
        ...
        // 이펙트 진행
        for (const e of this.effects) e.progress += dt / this.flashDurationMs;
        this.effects = this.effects.filter(e => e.progress < 1);

        // 유닛 플래시 감쇠
        for (const u of this.units.values()) {
            if (u.flashMs > 0) u.flashMs = Math.max(0, u.flashMs - dt);
        }
    ...
    private stepForward(): void {
        ...
        switch (log.actionType) {
            ...
            case 'ATTACK': {
                // 피격자 위치에 피격 이펙트
                if (actor) actor.flashMs = this.flashDurationMs;
        ...
        if (log.actionType !== 'TURN') {
            this.cb.addLog(
                `▶ T${log.turn} ${log.officerId} ${log.actionType}` +
