 ...
 */
export function getReputationRecruitModifier(store: GameStore, factionId: string | null): number {
 ...
 */
export function getReputationDiplomacyModifier(store: GameStore, factionId: string | null): number {
    if (!factionId) return 0;
    const faction = store.getFaction(factionId);
    if (!faction) return 0;
    const leader = store.getOfficer(faction.leaderId);
 ...
 */
export function applyReputationToRecruitChance(store: GameStore, factionId: string | null, baseChance: number): number {
    const modifier = getReputationRecruitModifier(store, factionId);
    return Math.max(0.05, Math.min(0.95, baseChance + modifier));
}

/** 평판 보정 설명 문구 (UI 표시용) */
export function describeReputationModifier(modifier: number): string {
    if (modifier >= 0.05) return `✨ 명성 보정 +${Math.round(modifier * 100)}%p`;
    if (modifier <= -0.05) return `⚠️ 악명 페널티 ${Math.round(modifier * 100)}%p`;
    if (modifier > 0) return `명성 보정 +${Math.round(modifier * 100)}%p`;
    if (modifier < 0) return `악명 페널티 ${Math.round(modifier * 100)}%p`;
    return '';
}
