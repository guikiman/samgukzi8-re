...
export class ActionValidator {
  validate(context: ActionValidationContext, officers: Map<string, OfficerState>, cities: Map<string, CityState>): { valid: boolean; reason?: string } {
    const officer = officers.get(context.officerId);
    if (!officer) return { valid: false, reason: "무장이 존재하지 않습니다." };
    if (context.requiresAlive && !officer.alive) return { valid: false, reason: "무장이 사망했습니다." };

    if (context.requiresCity) {
      const city = cities.get(context.targetId);
      if (!city) return { valid: false, reason: "목적지 도시가 존재하지 않습니다." };
    }
