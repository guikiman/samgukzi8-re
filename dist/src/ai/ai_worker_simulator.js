 ...
 *   7. 기본 행동   — 휴식
 */
export function calculateOfficerDecision(officer, cities, factions) {
    if (officer.isPlayer)
        return null;
    ...
    if (officer.loyalty < 30 && officer.ambition > 50) {
        return makeDecision(officer, 'REST', 0.99, `충성도 ${officer.loyalty} 붕괴 — 하야 검토`, {});
    }
    ...
    if (warTarget && aggression > 0.62 && homeCity && homeCity.foodStores > 500) {
        ...
        return makeDecision(officer, 'BATTLE', Math.min(0.99, priority), `${warTarget.name} 공격 검토 (공격성향 ${aggression.toFixed(2)}, 병량 충분)`, { targetCityId: warTarget.id });
    }
    // ── 3. 병량 위기 — 보급 우선 [121-130] ──
    if (homeCity && homeCity.foodStores < 200 && faction && faction.food < 500) {
        return makeDecision(officer, 'DOMESTIC', 0.9, `${homeCity.name} 병량 위기 — 개간 시급`, { targetCityId: homeCity.id });
    }
    // ── 4. 군주 성향 가중 징병 ──
    if (homeCity && shouldDraft(officer, homeCity, tw)) {
        return makeDecision(officer, 'RECRUITMENT', Math.min(0.9, 0.8 * tw.DRAFT), `${homeCity.name} 징병 (방어도 ${homeCity.defense})`, { targetCityId: homeCity.id });
    }
    // ── 5. 도시 상황 기반 내정 분기 ──
    if (homeCity) {
...
function shouldDraft(officer, city, tw) {
    // 패도 성향일수록 높은 방어도에서도 징병 (임계치 = 60 × DRAFT 배율)
    const draftThreshold = 60 * tw.DRAFT;
    if (city.defense < draftThreshold && officer.stats.leadership > 55)
        return true;
...
function chooseDomesticAction(officer, city, tw) {
    const weakest = Math.min(city.development, // 개발도
    Math.min(100, city.funds / 10), // 상업 환산
    Math.min(100, city.foodStores / 10));
    if (weakest >= 85)
        return null; // 내정 여유 — 다른 행동 기회
    const priority = Math.min(0.8, (0.4 + (100 - weakest) / 200) * tw.DOMESTIC);
    return makeDecision(officer, 'DOMESTIC', priority, `${city.name} 내정 (최약 지표 ${weakest.toFixed(0)})`, { targetCityId: city.id });
}
/** 가장 약한 적대 도시 탐색 — 전쟁 중 세력 기준 */
function findWeakestEnemyCity(faction, cities) {
    if (!faction || faction.atWarWith.length === 0)
        return null;
    const warSet = new Set(faction.atWarWith);
    let weakest = null;
    for (const city of Object.values(cities)) {
        if (!city.ownerId || !warSet.has(city.ownerId))
            continue;
        if (!weakest || city.defense < weakest.defense)
            weakest = city;
    }
    return weakest;
}
function makeDecision(officer, actionType, priority, reasoning, payload) {
    return {
 ...
 */
export async function runAITurn(payload, onFactionBatch, yieldIntervalMs = 5) {
...
//# sourceMappingURL=ai_worker_simulator.js.map