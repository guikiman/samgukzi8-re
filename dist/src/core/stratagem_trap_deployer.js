...
export class StratagemTrapDeployer {
     ...
     * @returns 배치 성공 여부
     */
    deployTrap(q, r, type, authorId, intel, turn) {
        if (this.traps.size >= this.MAX_TRAPS)
            return false;
        const coordKey = `${q},${r}`;
        if (this.traps.has(coordKey))
            return false; // 중복 배치 불가
     ...
     */
    getTrap(coordKey, isOwner) {
        const trap = this.traps.get(coordKey);
        if (!trap)
            return null;
        if (!isOwner)
            return null; // 적에게 완전 블라인드
        return trap;
    }
    /** 적에게 보이는 반투명 함정 목록 (탐지 스탯 높을 경우만) */
    getSemiVisibleTraps(intel) {
        const result = [];
...
//# sourceMappingURL=stratagem_trap_deployer.js.map