...
export function computeVagrantStrength(store, factionId) {
 ...
 */
export function processVagrantMonthlyActions(store) {
    const results = [];
    for (const faction of store.getAllFactions()) {
        ...
        const freeOfficers = store.getAllOfficers().filter(o => o.factionId === null && o.status === 'FREE');
 ...
 */
export function resolvePlayerRaid(store, factionId, targetCityId) {
    const faction = store.getFaction(factionId);
    if (!faction)
    ...
    if (city.ownerId === factionId)
        return { success: false, message: '이미 자기 세력 도시입니다.' };
...
//# sourceMappingURL=vagrant_monthly_actions.js.map