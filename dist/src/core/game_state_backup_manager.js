...
function diffRecord(current, previous, comparator) {
    const delta = {};
...
function deepEqual(a, b, depth = 0) {
    ...
    if (aArr) {
        ...
        for (let i = 0; i < arrA.length; i++) {
            if (!deepEqual(arrA[i], arrB[i], depth + 1))
                return false;
        }
        return true;
    }
    ...
    for (const key of keysA) {
        if (!Object.prototype.hasOwnProperty.call(objB, key))
            return false;
        if (!deepEqual(objA[key], objB[key], depth + 1))
            return false;
    }
    return true;
}
function applyRecordDelta(target, delta) {
...
export class CompressionCodec {
    ...
    static base64Encode(bytes) {
        ...
        for (let i = 0; i < bytes.length; i += 3) {
            ...
            if (i + 1 < bytes.length)
                r += CompressionCodec.BASE64_CHARS[(t >> 6) & 0x3F];
            if (i + 2 < bytes.length)
                r += CompressionCodec.BASE64_CHARS[t & 0x3F];
        }
        return r;
    }
    ...
    codeUnitsToString(u) {
        ...
        for (let i = 0; i < u.length; i++)
            c[i] = String.fromCharCode(u[i]);
        return c.join('');
    }
}
CompressionCodec.MIN_MATCH = 3;
CompressionCodec.MAX_MATCH = 128;
CompressionCodec.WINDOW_SIZE = 32768;
CompressionCodec.BASE64_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
CompressionCodec.BASE64_MAP = {};
(() => {
    for (let i = 0; i < 64; i++) {
...
export class TimeMachine {
    ...
    rollbackToSnapshot(store, targetTurn) {
        ...
        let bi = ti;
        while (bi >= 0 && !this.nodes[bi].fullSnapshot)
            bi--;
        if (bi < 0)
            return this.rebuildFromStart(store, ti);
    ...
    exportCompressedSave(store, label) {
    ...
    importCompressedSave(store, blob) {
        if (this.computeChecksum(blob.compressed) !== blob.checksum) {
            return false;
        }
        try {
            store.restoreSnapshot(JSON.parse(this.codec.decompress(blob.compressed)));
            return true;
        }
        catch {
            return false;
    ...
    loadFromLocalStorage(slot = 0) {
        try {
            const raw = localStorage.getItem(`sangkukzi_backup_${slot}`);
            if (!raw)
                return false;
            const d = JSON.parse(this.codec.decompress(raw));
            this.nodes = d.nodes;
    ...
    clearAllSlots() {
        for (let i = 0; i < 10; i++)
            localStorage.removeItem(`sangkukzi_backup_${i}`);
    }
    compact(keepRatio = 0.5) {
    ...
    canRedo() { return this.currentIndex < this.nodes.length - 1; }
    getCurrentTurn() { return this.currentIndex >= 0 ? this.nodes[this.currentIndex].turnCount : -1; }
    getHistoryLength() { return this.nodes.length; }
    getHistorySummary() {
    ...
    dispose() { this.nodes = []; this.currentIndex = -1; this.baseSnapshot = null; }
    restoreAtCurrentIndex(store) {
...
export class GameStateBackupManager {
    ...
    initialize(store) { this.store = store; }
    recordSnapshot(label, commands = []) {
        if (!this.store)
            return;
    ...
    undo() {
        if (!this.store)
            return { success: false, restoredTurnCount: 0, restoredTime: { year: 0, month: 0 }, commandsRolledBack: 0, message: 'GameStore가 초기화되지 않았습니다' };
        if (!this.timeMachine.canUndo())
            return { success: false, restoredTurnCount: 0, restoredTime: { year: 0, month: 0 }, commandsRolledBack: 0, message: '되돌릴 기록이 없습니다' };
    ...
    redo() {
        if (!this.store)
            return { success: false, restoredTurnCount: 0, restoredTime: { year: 0, month: 0 }, commandsRolledBack: 0, message: 'GameStore가 초기화되지 않았습니다' };
        if (!this.timeMachine.canRedo())
            return { success: false, restoredTurnCount: 0, restoredTime: { year: 0, month: 0 }, commandsRolledBack: 0, message: '앞으로 되돌릴 기록이 없습니다' };
        return this.timeMachine.redo(this.store);
    }
    rollbackToSnapshot(targetTurn) {
        if (!this.store)
            return { success: false, restoredTurnCount: 0, restoredTime: { year: 0, month: 0 }, commandsRolledBack: 0, message: 'GameStore가 초기화되지 않았습니다' };
        return this.timeMachine.rollbackToSnapshot(this.store, targetTurn);
    }
    exportCompressedSave(label) {
        if (!this.store)
            return null;
        return this.timeMachine.exportCompressedSave(this.store, label);
    }
    importCompressedSave(blob) {
        if (!this.store)
            return false;
        return this.timeMachine.importCompressedSave(this.store, blob);
    }
    persistToLocalStorage(slot = 0) { return this.timeMachine.persistToLocalStorage(slot); }
    loadFromLocalStorage(slot = 0) { return this.timeMachine.loadFromLocalStorage(slot); }
    compact(keepRatio = 0.5) { this.timeMachine.compact(keepRatio); }
    getStats() { return this.timeMachine.getStats(); }
    canUndo() { return this.timeMachine.canUndo(); }
    canRedo() { return this.timeMachine.canRedo(); }
    getHistorySummary() { return this.timeMachine.getHistorySummary(); }
    dispose() { this.store = null; this.timeMachine.dispose(); }
}
//# sourceMappingURL=game_state_backup_manager.js.map