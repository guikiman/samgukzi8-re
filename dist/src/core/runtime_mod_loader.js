...
export class SchemaValidator {
    constructor() {
        this.officerMaxStats = 100;
        this.maxYear = 399;
        this.minYear = 100;
        this.validGenders = new Set(['M', 'F']);
        this.validPersonalities = new Set([
            'DIGNIFIED', 'IMPULSIVE', 'COURAGEOUS', 'CAUTIOUS',
            'CRUEL', 'RIGHTEOUS', 'VAIN', 'SERENE', 'WILY', 'GULLIBLE',
        ]);
        this.validCategories = new Set(['historical', 'fictional', 'character', 'disaster', 'chain_step']);
        this.validChainTypes = new Set(['single', 'multi_turn', 'branching']);
        this.existingOfficerIds = new Set();
        this.existingFactionIds = new Set();
        this.existingCityIds = new Set();
        this.existingEventIds = new Set();
    }
    /** 기존 게임 DB의 ID 목록을 로드 (참조 무결성 검증용) */
    loadExistingIds(store) {
    ...
    validateOfficer(o, path, isNew) {
        ...
        for (const field of ['loyalty', 'ambition', 'morality', 'greed']) {
            const val = o[field];
...
export class ModRegistry {
    ...
    /** 로드된 모든 모드 ID */
    get loadedModIds() { return this.loadOrder; }
    /** 특정 모드 메타 조회 */
    getMod(id) { return this.mods.get(id) ?? null; }
    /** 로드된 모드 수 */
    get count() { return this.mods.size; }
}
// ============================================================
// [3] RuntimeModLoader
// ============================================================
...
export class RuntimeModLoader {
     ...
     * @returns           로드 결과
     */
    async loadModFromFile(jsonString) {
        const errors = [];
        const warnings = [];
        const stats = {
            officersLoaded: 0,
            ...
            for (const o of data.officers) {
                ...
                if (existing) {
                    ...
                    this.store.updateOfficer(o.id, {
                        ...
                        ...(o.personality ? { personality: o.personality } : {}),
                    ...
                    this.store.addOfficer({
                        ...
                        personality: (o.personality ?? 'DIGNIFIED'),
    ...
    get validatorRef() { return this.validator; }
}
//# sourceMappingURL=runtime_mod_loader.js.map