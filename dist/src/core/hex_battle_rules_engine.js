 ...
 *  전장 규칙을 통합한 전용 엔진)
 */
export function makeHexTile(q, r, terrainType = 'PLAINS', elevation = 0) {
...
export class HexBattleRulesEngine {
     ...
     */
    searchPath(start, goal, unitType, gridMap, enemyPositions) {
            ...
            if (n > 1) {
                ...
                for (;;) {
                    ...
                    if (l < heapF.length && heapF[l] < heapF[smallest])
                        smallest = l;
                    if (rgt < heapF.length && heapF[rgt] < heapF[smallest])
                        smallest = rgt;
                    if (smallest === i)
                        break;
    ...
    isInZOC(q, r, enemyPositions) {
        for (const [nq, nr] of this.getNeighbors(q, r)) {
            if (enemyPositions.has(key(nq, nr)))
                return true;
        }
        return false;
    }
...
//# sourceMappingURL=hex_battle_rules_engine.js.map