 ...
 */
function findSwornBrotherEdge(store, aId, bId) {
 ...
 * 두 무장이 의형제인지 확인
 */
export function areSwornBrothers(store, aId, bId) {
    if (aId === bId)
        return false;
 ...
 */
export function findRescuers(store, captiveId) {
    ...
    for (const edge of edges) {
        ...
        if (!brother)
            continue;
        if (!brother.runtime.isAlive)
            continue;
        if (isCaptive(store, brother.id))
            continue; // 구하러 갈 의형제도 포로면 불가
 ...
 */
export function judgeRescue(might, roll, chance = BASE_RESCUE_CHANCE + might * MIGHT_RESCUE_FACTOR) {
 ...
 */
export function processMonthlySwornBrotherRescues(store) {
    const rescued = [];
    const messages = [];
    ...
    for (const captive of captives) {
        ...
        if (!judgeRescue(rescuer.stats.might, Math.random()))
            continue;
...
//# sourceMappingURL=sworn_brother_rescue_system.js.map