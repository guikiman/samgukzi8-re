export class HistoricalEventSystem {
    ...
    checkEvents(currentYear, currentMonth, officerStatus, factionStatus) {
        const triggered = [];
        for (const event of this.events) {
            if (event.triggered && event.oneTimeOnly)
                continue;
    ...
    evaluateCondition(cond, currentYear, currentMonth, officerStatus, factionStatus) {
        switch (cond.type) {
            case "year_reached":
                return currentYear >= cond.params.year && currentMonth >= (cond.params.month ?? 1);
            case "officer_alive":
                return officerStatus.get(cond.params.officerId) ?? false;
            case "officer_dead":
                return !(officerStatus.get(cond.params.officerId) ?? true);
            case "faction_exists":
                return factionStatus.get(cond.params.factionId) ?? false;
            case "faction_destroyed":
                return !(factionStatus.get(cond.params.factionId) ?? true);
            default:
                return false;
        }
    }
    applyEffects(event) {
        for (const effect of event.effects) {
            if (effect.type === "event_flag") {
                this.globalFlags.set(effect.params.flag, true);
            }
        }
    }
    isFlagSet(flag) {
    ...
    getPendingEvents(currentYear, currentMonth) {
        return this.events.filter((e) => {
            if (e.triggered && e.oneTimeOnly)
                return false;
