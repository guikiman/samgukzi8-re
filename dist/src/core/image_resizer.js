...
export class ImageResizer {
     ...
     * 이미지 리사이징
     */
    resize(source, targetWidth, targetHeight, fit = "fill", filter = "bilinear") {
     ...
     * 캔버스 → Blob
     */
    toBlob(canvas, quality = 0.92) {
        return new Promise((resolve, reject) => {
            canvas.toBlob((blob) => {
                if (blob)
                    resolve(blob);
                else
                    reject(new Error("[ImageResizer] toBlob failed"));
            }, "image/png", quality);
        });
    }
    /**
     * 이미지 URL → 리사이즈된 캔버스
     */
    async resizeFromUrl(url, targetWidth, targetHeight, fit = "fill") {
        const img = new Image();
        img.src = url;
        await img.decode();
     ...
     * 메모리 사용량 추정 (바이트)
     */
    estimateMemory(width, height, bytesPerPixel = 4) {
