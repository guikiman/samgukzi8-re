...
// ============================================================
/** 내정 분야 열거형 */
export var DomesticTaskType;
(function (DomesticTaskType) {
    DomesticTaskType["COMMERCE"] = "COMMERCE";
    DomesticTaskType["FARMING"] = "FARMING";
    DomesticTaskType["TECHNOLOGY"] = "TECHNOLOGY";
    DomesticTaskType["PUBLIC_ORDER"] = "PUBLIC_ORDER";
    DomesticTaskType["REPAIR"] = "REPAIR";
})(DomesticTaskType || (DomesticTaskType = {}));
// ============================================================
...
export class DomesticScheduler {
     ...
     */
    autoAssign(cityId, slots, excludeIds) {
     ...
     */
    registerAssignment(cityId, assignment) {
        const city = this.store.getCity(cityId);
        if (!city)
            throw new Error(`[Scheduler] 도시(ID: ${cityId})가 존재하지 않습니다.`);
        // 1. 최소 비용 검증
        if (assignment.allocatedFunds < DomesticScheduler.MIN_FUNDS) {
            throw new Error(`[Scheduler] 최소 할당 비용(${DomesticScheduler.MIN_FUNDS}금)이 부족합니다. (할당: ${assignment.allocatedFunds})`);
        }
        // 2. 도시 자원 검증
        if (city.funds < assignment.allocatedFunds) {
            throw new Error(`[Scheduler] 도시(${city.name})의 보유 금(${city.funds})이 할당 비용(${assignment.allocatedFunds})보다 부족합니다.`);
        }
        // 3. 무장 행동력 검증
     ...
     *   else        → SUCCESS  (×1.0)
     */
    calculateTaskEffect(city, assignment, officers) {
        ...
        const logMessage = `[${city.name}] ${names} 무장이 ${assignment.taskType} 임무 수행. ` +
    ...
    getTargetStatDetails(city, type) {
    ...
    applyStatChange(city, type, increment) {
        const ds = city.developmentStats;
        switch (type) {
            case DomesticTaskType.COMMERCE:
     ...
     */
    checkAutoTransition(cityId) {
        const city = this.store.getCity(cityId);
        if (!city || !city.developmentStats)
            return null;
        const ds = city.developmentStats;
    ...
    scanAllAutoTransitions() {
        ...
        for (const city of this.store.getAllCities()) {
            ...
            if (rule)
                results.push({ cityId: city.id, rule });
        }
        return results;
    }
}
/** 최소 할당 비용 */
DomesticScheduler.MIN_FUNDS = 50;
/** 행동력 소모량 */
DomesticScheduler.AP_COST = 20;
/** 기본 성공 확률 문턱 */
DomesticScheduler.CRITICAL_THRESHOLD = 0.9;
DomesticScheduler.FAILURE_THRESHOLD = 0.1;
//# sourceMappingURL=domestic_scheduler.js.map