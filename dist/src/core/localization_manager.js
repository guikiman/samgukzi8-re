...
export class LocalizationManager {
    constructor(options = {}) {
        ...
        this.fontState = new Map();
        this.fontLoadPromises = new Map();
        this.onLanguageChanged = [];
        this._initialized = false;
        this._layoutAdjustment = {
            scale: 1,
            lineHeight: 1.5,
            letterSpacing: 0,
    ...
    async init() {
        ...
        if (this.preloadLanguages.length > 0) {
            await Promise.all(this.preloadLanguages
    ...
    async setLanguage(code) {
        if (code === this.currentLanguage && this.currentPack)
            return;
    ...
    async loadFont(code) {
        ...
        if (this.fontState.get(fontFamily) === 'LOADED')
            return true;
    ...
    /** 현재 레이아웃 조정 파라미터 */
    get layoutAdjustment() { return this._layoutAdjustment; }
    /** CSS 변수로 레이아웃 파라미터 적용 */
    applyLayoutToCSS(root = document.documentElement) {
 ...
 * @returns      로케일 기반 문자열
 */
export function formatLocaleNumber(value, locale) {
    const localeMap = {
        'ko-KR': 'ko-KR',
        'zh-CN': 'zh-CN',
        'zh-TW': 'zh-TW',
 ...
 */
export function formatPlural(count, singular, plural, locale) {
...
//# sourceMappingURL=localization_manager.js.map