 ...
 */
export function areNemesis(store, aId, bId) {
    if (aId === bId)
        return false;
 ...
 */
export function judgeVengeance(store, aId, bId, roll, chance = VENGEANCE_CHANCE) {
    ...
    const kind = actor.stats.might >= actor.stats.intelligence ? 'DUEL' : 'DEBATE';
 ...
 */
export function executeVengeance(store, actorId, targetId, kind) {
 ...
 */
export function tryVengeanceOnEncounter(store, aId, bId, roll = Math.random()) {
...
export function judgeVengeanceOnly(store, aId, bId, roll = Math.random()) {
 ...
 */
export function startVengeanceGame(store, actorId, targetId, kind) {
    const actor = store.getOfficer(actorId);
    const target = store.getOfficer(targetId);
    if (!actor || !target)
        throw new Error('복수 이벤트 무장을 찾을 수 없습니다');
    if (kind === 'DUEL') {
        const duel = new DuelMinigame();
        duel.startDuel(actor.id, target.id, actor.stats, target.stats);
        return duel;
 ...
 */
export function finishVengeance(store, actorId, targetId, success) {
    const actor = store.getOfficer(actorId);
    if (!actor)
        return NO_VENGEANCE;
    const { year, month } = store.getGlobalState().time;
 ...
 */
export function processMonthlyVengeance(store) {
    const outcomes = [];
    const officers = store.getAllOfficers().filter(o => o.factionId !== null && o.status !== 'FREE' && o.runtime.isAlive);
...
export function applyVengeanceToUnits(outcome, allies, enemies) {
    if (!outcome.triggered)
        return { allyLog: null, enemyLog: null };
...
//# sourceMappingURL=vengeance_system.js.map