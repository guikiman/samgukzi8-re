...
export class AudioNodePool {
    ...
    disposePanner(idx) {
        try {
            this.pannerNodes[idx]?.disconnect();
        }
        catch { /* */ }
    }
    ...
    disposeFilter(idx) {
        try {
            this.filterNodes[idx]?.disconnect();
        }
        catch { /* */ }
    }
    /** 풀의 GainNode dispose */
    disposeGain(idx) {
        try {
            this.gainNodes[idx]?.disconnect();
        }
        catch { /* */ }
    }
    /** 모든 노드 정리 */
    disposeAll() {
        for (const p of this.pannerNodes)
            try {
                p.disconnect();
            }
            catch { /* */ }
...
export class TacticalAudioEngine {
    ...
    async init() {
        try {
            ...
            if (this.preloadBuffers.length > 0) {
                await Promise.all(this.preloadBuffers.map(b => this.loadBuffer(b.id, b.url)));
            }
            // 기본 앰비언트 초기화
    ...
    updateListenerVec3(pos, forward, up) {
     ...
     */
    playSound(options) {
        if (!this.ctx || !this.pool || !this.masterGain)
            return null;
    ...
    // 내부: 인스턴스 해제
    // ============================================================
    releaseInstance(instance, poolIdx) {
        try {
...
//# sourceMappingURL=tactical_audio_engine.js.map