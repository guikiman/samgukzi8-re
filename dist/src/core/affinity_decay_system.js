...
export class AffinityDecaySystem {
    ...
    resetToDefault() {
        ...
        const p = ["AGGRESSIVE", "CALM", "CAUTIOUS", "TIMID", "LOYAL", "AMBITIOUS", "RIGHTEOUS", "GREEDY"];
        for (const a of p) {
            const row = new Map();
            for (const b of p)
                row.set(b, 0);
    ...
    setCustomCompatibility(a, b, factor) {
        this.set(a, b, factor);
    }
    getCompatibilityFactor(a, b) {
        return this.compatibility.get(a.personality)?.get(b.personality) ?? 0;
    }
    calculateDecay(officerA, officerB, currentAffinity, turnsSinceLastInteraction) {
        ...
        if (projected > maxAffinity)
            decay = maxAffinity - currentAffinity;
        if (projected < minAffinity)
            decay = minAffinity - currentAffinity;
        return decay;
    }
}
//# sourceMappingURL=affinity_decay_system.js.map