...
export class AnnualReportEngine {
     ...
     * @returns 생성된 연간 보고서
     */
    generateReport(factions, year) {
        const factionReports = [];
     ...
     * 단일 세력의 연간 보고서를 생성한다.
     */
    buildFactionReport(faction, previous, year) {
     ...
     * 글로벌 요약 정보를 생성한다.
     */
    buildGlobalSummary(factionReports) {
        let mostPowerful = '';
        let maxCities = 0;
        let fastestGrowing = '';
        ...
        const totalCities = factionReports.reduce((sum, r) => sum + r.territory.currentCityCount, 0);
        return {
...
//# sourceMappingURL=annual_report_engine.js.map