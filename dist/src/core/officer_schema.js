export function sanitizeAndValidateOfficer(raw) {
    if (typeof raw.id !== 'string' || !raw.id.startsWith('KOEI_'))
        return null;
    if (typeof raw.name !== 'string')
        return null;
    if (typeof raw.affinity !== 'number' || raw.affinity < 0 || raw.affinity > 149)
        return null;
    if (!raw.stats || typeof raw.stats !== 'object')
        return null;
    const reqStats = ['leadership', 'might', 'intelligence', 'politics', 'charisma'];
    if (!reqStats.every(k => typeof raw.stats[k] === 'number'))
        return null;
    const processed = {
        ...raw,
        runtime: raw.runtime ?? {
...
//# sourceMappingURL=officer_schema.js.map