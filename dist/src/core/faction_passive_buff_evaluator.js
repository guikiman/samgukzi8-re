...
const EMPEROR_ENDORSEMENT_EFFECTS = {
...
export class FactionPassiveBuffEvaluator {
     ...
     */
    evaluateAllFactions(factions, turn) {
     ...
     */
    evaluateSingleFaction(faction, _turn) {
        const aggregated = {
            GOLD_PRODUCTION: 0,
            FOOD_PRODUCTION: 0,
            RECRUITMENT_RATE: 0,
     ...
     * 기술 레벨에 따른 패시브 버프 적용
     */
    applyTechBuffs(faction, aggregated) {
     ...
     * 평판 기반 버프 적용
     */
    applyReputationBuffs(faction, aggregated) {
        let bestMatch = null;
     ...
     * 황제 옹립 버프 적용
     */
    applyEmperorEndorsementBuffs(level, aggregated) {
     ...
     * 세력 정책 기반 버프 적용
     */
    applyPolicyBuffs(faction, aggregated) {
     ...
     * 버프 상태를 레지스트리에 기록한다.
     */
    recordBuffState(factionId, effects, turn) {
        ...
        for (const [stat, value] of Object.entries(effects)) {
            if (value !== 0) {
                activeBuffs.push({
                    ...
                    source: 'TECHNOLOGY',
                    effects: [{ stat: stat, operation: value > 0 ? 'ADD' : 'MULTIPLY', value: Math.abs(value) }],
                    duration: -1,
                    remainingTurns: -1,
                    isActive: true,
                });
     ...
     */
    getAggregatedEffects(factionId) {
...
//# sourceMappingURL=faction_passive_buff_evaluator.js.map