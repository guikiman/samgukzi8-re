...
export class WebGLShaderManager {
    constructor() {
        ...
        this.vertexShader = this.compileShaderSource(FULLSCREEN_VERTEX_SHADER, WebGL2RenderingContext.VERTEX_SHADER);
    }
    // ============================================================
    // WebGL 초기화
    // ============================================================
    ...
    // 셰이더 리소스 관리
    // ============================================================
    compileShaderSource(source, type) {
        if (!this.gl)
    ...
    compileProgram(name, fragmentSource) {
        if (!this.gl)
            throw new Error("WebGL not initialized");
        const gl = this.gl;
        const fragmentShader = this.compileShaderSource(fragmentSource, gl.FRAGMENT_SHADER);
    ...
    createRippleTexture(gl, w, h) {
        const data = new Uint8Array(w * h * 4);
        const texture = gl.createTexture();
        if (!texture)
            throw new Error("Failed to create ripple texture");
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, w, h, 0, gl.RGBA, gl.UNSIGNED_BYTE, data);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    ...
    // 리플 텍스처 업데이트 (CPU→GPU)
    // ============================================================
    updateRippleTexture(ripples) {
        if (!this.gl || !this.rippleTexture)
            return;
        const gl = this.gl;
        const w = 256;
        const h = 256;
    ...
    // 셰이더 렌더링
    // ============================================================
    renderEffect(effect, sourceTexture, uniforms) {
        const gl = this.gl;
        const program = this.programs.get(effect);
        if (!gl || !program || !this.quadVAO)
            return;
...
export class VisualEffectRenderer {
     ...
     */
    initialize(canvas, sourceCanvas) {
        this.canvas = canvas;
    ...
    // 이펙트 큐
    // ============================================================
    enqueueTransitionInkBleed(durationMs = 1000) {
        this.effectQueue.push({
            type: "inkBleed",
            progress: 0,
        });
    }
    triggerChromaticAberration(intensity = 1.0) {
        this.effectQueue.push({
            type: "chromaticAberration",
            intensity,
        });
    }
    triggerWaterRipple(ripplePoints) {
        this.effectQueue.push({
            type: "waterRipple",
            time: 0,
            pulseAlpha: 0,
    ...
    // ============================================================
    update(dt) {
        if (!this.isReady || this.effectQueue.length === 0)
            return;
        // 소스 캔버스 → WebGL 텍스처 복사
    ...
    // ============================================================
    copySourceToTexture() {
        if (!this.gl || !this.srcCanvas || !this.srcCtx)
            return;
        const gl = this.gl;
        if (!this.srcTexture) {
...
//# sourceMappingURL=visual_effect_renderer.js.map