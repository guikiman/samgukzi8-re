export class AllianceSystem {
    ...
    proposeTreaty(proposerId, targetId, type, proposerDiplomacy, proposerPersonality, powerRatio, commonEnemy) {
    ...
    breakTreaty(factionId, treaty) {
        this.treaties = this.treaties.filter((t) => t !== treaty);
    }
    getActiveTreaties(factionId) {
        return this.treaties.filter((t) => t.between.includes(factionId));
    }
    isAtWar(factionId) {
        return this.getActiveTreaties(factionId).some((t) => t.type === "MILITARY_COALITION");
    }
    formCoalitionAgainst(targetId, proposerId, potentialMembers) {
        const formed = [];
...
//# sourceMappingURL=alliance_system.js.map