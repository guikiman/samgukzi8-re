...
export function checkInteraction(store, actorId, targetId, kind) {
    if (actorId === targetId)
        return { ok: false, reason: '자기 자신과는 상호작용할 수 없습니다.' };
    const actor = store.getOfficer(actorId);
    const target = store.getOfficer(targetId);
    if (!actor)
        return { ok: false, reason: '초빙자(행동 무장)를 찾을 수 없습니다.' };
    if (!target)
        return { ok: false, reason: '대상 무장을 찾을 수 없습니다.' };
    if (kind === 'GIFT') {
        const faction = actor.factionId ? store.getFaction(actor.factionId) : null;
        if (!faction)
            return { ok: false, reason: '소속 세력이 없어 증정할 수 없습니다.' };
        if (faction.gold < GIFT_COST)
            return { ok: false, reason: `국고가 부족합니다 (증정 비용 ${GIFT_COST}金).` };
    }
    if (kind === 'CHAT') {
        ...
        if (chattedThisMonth)
            return { ok: false, reason: '이번 달에는 이미 대화했습니다 (월 1회).' };
    }
    return { ok: true };
}
/**
 ...
 */
function applyAffinity(store, actorId, targetId, delta, event) {
    const { year, month } = store.getGlobalState().time;
...
function recordInteractionHistory(store, actor, target, kind, affinityDelta) {
    const buffer = store.interactionHistory;
    if (!buffer)
        return;
    const { year, month } = store.getGlobalState().time;
    buffer.push({
 ...
 */
export function executeInteraction(store, actorId, targetId, kind) {
...
export function getAffinityBetween(store, aId, bId) {
    ...
    if (reverse)
        return reverse.affinity;
    return 0;
}
//# sourceMappingURL=officer_interaction_system.js.map