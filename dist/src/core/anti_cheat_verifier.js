...
export class ChecksumMismatchError extends Error {
    constructor(matchId, playerId, serverChecksum, clientChecksum, desyncTurn, desyncAction) {
        super(`[AntiCheat] Checksum mismatch in match ${matchId} (player ${playerId}): ` +
...
export class AntiCheatVerifier {
    ...
    verify(actionLog) {
        ...
        if (!match) {
            ...
            throw new ChecksumMismatchError(actionLog.matchId, actionLog.playerId, serverChecksum, clientChecksum, desyncTurn ?? 0, desyncAction ?? 0);
        }
    ...
    replayAction(entry, prng) {
        switch (entry.actionType) {
            ...
            default: {
                this.log(`  [WARN] Unknown action type: ${entry.actionType}`);
                break;
            }
        }
    }
     ...
     *   - 현재 PRNG state
     */
    computeActionChecksum(entry, prng) {
 ...
 * @returns    32비트 체크섬
 */
export function computeActionLogChecksum(log) {
    let hash = 0x811C9DC5; // FNV offset basis
    // seed
...
/** FNV-1a: Int32 업데이트 */
function fnvUpdateInt(hash, value) {
    let h = hash;
    const bytes = new Uint8Array(4);
...
/** FNV-1a: String 업데이트 */
function fnvUpdateString(hash, value) {
    let h = hash;
...
//# sourceMappingURL=anti_cheat_verifier.js.map