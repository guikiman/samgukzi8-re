...
export class AllianceLeagueManager {
     ...
     */
    proposeLeague(initiatorFactionId, targetFactionId, memberFactionIds) {
     ...
     * @param approve - 찬성 여부
     */
    voteOnLeague(leagueId, factionId, approve) {
        const league = this.leagues.get(leagueId);
        if (!league)
            return false;
        if (!league.memberFactionIds.includes(factionId))
            return false;
    ...
    /** 활성 연합 확인 */
    isLeagueActive(leagueId) {
        const league = this.leagues.get(leagueId);
        return league?.isActive ?? false;
    }
    /** 특정 세력이 참여 중인 모든 연합 조회 */
    getLeaguesForFaction(factionId) {
        return Array.from(this.leagues.values()).filter(l => l.isActive && l.memberFactionIds.includes(factionId));
    }
    /** 연합 타겟인 세력 조회 */
    getLeaguesTargetingFaction(factionId) {
        return Array.from(this.leagues.values()).filter(l => l.isActive && l.targetFactionId === factionId);
    }
    ...
    /** 연합이 상호방어를 제공하는지 확인 */
    isMutualDefenseActive(defenderFactionId, attackerFactionId) {
        for (const league of this.leagues.values()) {
            if (!league.isActive)
                continue;
            if (league.targetFactionId !== attackerFactionId)
                continue;
            if (league.memberFactionIds.includes(defenderFactionId))
                return true;
        }
        return false;
    }
...
//# sourceMappingURL=alliance_league_system.js.map