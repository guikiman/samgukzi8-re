...
export class DeathSuccessionManager {
    constructor() {
        ...
        this.currentDate = 184;
    }
    setCurrentDate(date) {
        this.currentDate = date;
    }
    /** 무장 사망 처리 */
    processDeath(officerId, factionId, cause, age) {
        const event = { officerId, factionId, cause, date: this.currentDate, age };
        this.deathEvents.push(event);
        return event;
    }
    /** 후계자 선정 */
    selectHeir(factionId, candidates) {
    ...
    /** 후계자 승계 */
    triggerSuccession(factionId, deceasedOfficerId, successorId, officers, cause) {
    ...
    /** 자연사 확률 계산 */
    calculateNaturalDeathProbability(age) {
        if (age < 20)
            return 0.001;
        if (age < 40)
            return age / 10000;
...
//# sourceMappingURL=death_succession_system.js.map