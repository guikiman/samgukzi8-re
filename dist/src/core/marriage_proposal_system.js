export class MarriageProposalSystem {
    canPropose(ctx) {
        if (ctx.targetAge < 16)
            return { allowed: false, reason: "상대방이 너무年轻합니다." };
        if (ctx.proposerAge < 16)
            return { allowed: false, reason: "아직 혼인할 나이가 아닙니다." };
        if (ctx.targetAge > 50)
            return { allowed: false, reason: "상대방이 이미 혼인하기에 늦은 나이입니다." };
        return { allowed: true };
    }
    calculateSuccessChance(ctx) {
        let chance = 0;
        ...
        if (!ctx.mutualFaction)
            chance -= 0.2;
        return Math.max(0, Math.min(0.95, chance));
    }
    generateProposalOutcome(success, proposerName, targetName) {
        if (success) {
...
//# sourceMappingURL=marriage_proposal_system.js.map