...
export async function loadScenarios() {
    ...
    if (!res.ok)
        throw new Error(`시나리오 데이터 로드 실패: ${res.status}`);
    const data = (await res.json());
    cachedScenarios = data;
    return data;
}
export function getCachedScenarios() {
    return cachedScenarios ?? [];
}
/** 무장 아이디 → 한글 이름 (사전에 없으면 아이디 그대로) */
export function getKnownOfficerName(id) {
...
function buildOfficer(id, cityId, factionId, year, isLeader) {
...
export function buildWorld(scenario, playerFactionIndex) {
    ...
    for (const [leaderId, companions] of Object.entries(companionMap)) {
        ...
        for (const compId of companions) {
            if (officers.some(o => o.id === compId))
                continue;
