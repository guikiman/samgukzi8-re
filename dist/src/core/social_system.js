...
export class SocialSystem {
     ...
     */
    getInteractCandidates(factionId, options = {}) {
        ...
        const candidates = allOfficers.map(o => {
            const affinity = this.getAffinityBetween(faction.leaderId, o.id);
            const rankDist = Math.abs(o.rank - (faction.lordRank ?? 9));
     ...
     */
    computeRansom(officerId, captorFactionId) {
        ...
        const rankWeights = { 1: 5, 2: 3, 3: 2, 4: 1.5, 5: 1.2, 6: 1, 7: 0.8, 8: 0.6, 9: 0.5 };
        const rankWeight = rankWeights[officer.rank] ?? 0.5;
