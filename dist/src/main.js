...
function renderCityDetailPanel(city, faction, switched) {
    ...
    if (faction) {
        ...
        if (cbSym)
            cdpFactionBadge.innerHTML += `<span class="cb-symbol">${cbSym}</span>`;
    }
    else {
...
function updateReplayViewer(dt) {
    ...
    ctx.fillText(replayViewer.isFinished ? '🏁 리플레이 종료 — 새로고침으로 다시 재생' : `🎬 리플레이 재생 중 (${replayViewer.actionCount} 액션 · ${replayViewer.speed}x)`, 10, 20);
    // 진행률 바 [461-480]
...
document.addEventListener('keydown', (e) => {
    if (!replayViewer || replayViewer.isFinished)
        return;
    if (e.key === ' ') {
        e.preventDefault();
...
function renderA11yPanel() {
    ...
    const cbRow = `<div class="a11y-row"><span class="a11y-label">세력 색</span><span class="a11y-opts">` +
        Object.keys(COLORBLIND_MODE_LABELS)
...
function renderMonthlyPortedSection(ported) {
...
function renderFactionList(s) {
    ...
    factionList.innerHTML = s.factions.map((f, i) => `<button class="faction-card" data-idx="${i}" style="--faction-color:${factionColor(f.color)}">
